...
const Step3Report = ({ report }) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text("InterviewIQ.AI Performance Report", pageWidth/2, currentY, {
      align : "center"
    })
    ...
    autoTable(doc,{
      ...
      body : questionWiseScore.map((q,i) => [
        ...
        `${q.score || 0}/10`,
        q.feedback || "Feedback not available"
      ]),
      styles : {
        fontSize : 9,
        cellPadding : 5,
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between'>
        ...
        <button 
        onClick={downloadPDF}
        className='px-4 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap'>
          Download PDF
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="space-y-6">
          ...
          initial = {{opacity : 0}}
          animate = {{opacity : 1}}
          className='bg-white rouned-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8'>
